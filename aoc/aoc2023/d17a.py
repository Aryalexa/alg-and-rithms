...
def next_moves(pos, _dir):
	# for ([u,d] or [r,l]) for (1-3) if isinside, return [(pos, dir, cost)]
	i,j = pos
	UD = (
		[((i-k, j), '^', hcost(i, -k, j, 0)) for k in range(1,4) if isinside((i-k, j))] + 
		[((i+k, j), 'v', hcost(i, k, j, 0)) for k in range(1,4) if isinside((i+k, j))] )
	LR = (
		[((i, j-k), '<', hcost(i, 0, j, -k)) for k in range(1,4) if isinside((i, j-k))] +
		[((i, j+k), '>', hcost(i, 0, j, k)) for k in range(1,4) if isinside((i, j+k))] )
...
"""
Dijkstra solution
It's a graph of states (pos, dir).
The state is used for distances {state:dist} and the heap [(cost, state)]

Actually two options
1. (pos, dir, streak) for 3 neighbors
2. (pos, dir) for STREAK*2 neighbors, STREAK=3

(I think the second aproach made part 2 easier)
"""
