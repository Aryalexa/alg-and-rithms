...
def how_many_fall(brick_name, visited):
	n = brick_nodes[brick_name]
	# print('NODE *',n, 'visited:', visited)
	visited.append(brick_name)
	if not n.has_above():
		return []
	affected = []
	for ch in n.above:
		other_supports = [x for x in brick_nodes[ch].below if x not in visited]
		# print('\t',ch,'supported by:', brick_nodes[ch].below,'->',other_supports)
		if len(other_supports) == 0:
			affected += [ch]
	new_affected = []
	for b in affected:
		new_affected += how_many_fall(b, visited + affected + new_affected)
	return list(set(affected + new_affected))
...
def part_2():
	res = 0
	keys = [x for x in brick_nodes.keys() if x !='F']
	for b in sorted(map(lambda x: int(x[1:]), keys)):
			#print('*******', b)
			x = how_many_fall(f'B{b}', ['F'])
			print(b, len(x))
			res += len(x)
	print('res',res)
